...
async function backfillPropertyImages() {
  const overwriteAll = process.argv.includes('--overwrite') || process.argv.includes('--force') || true;
  ...
  console.log(`📦 Found ${totalMatching} property records to update (Overwrite mode: ${overwriteAll}).`);
  ...
  console.log(`  Total Processed & Updated: ${totalUpdated}`);

...
const STATS = [
  { label: 'Properties Registered', value: 12400, suffix: '+' },
  { label: 'Transfers Completed',   value: 3890, suffix: '+' },
  { label: 'States Covered',         value: 18, suffix: '' },
  { label: 'Avg. Transfer Time',     value: 48, suffix: ' hrs' },
];
        ...
        onEnter: () => {
          gsap.fromTo(node, 
            ...
            { 
              ...
              snap: { innerHTML: 1 },
              onUpdate: function() {
                // Ensure it formats with commas if needed
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if(trustRef.current) {
        ...
        // Micro-animation for icons (bounce)
    ...
    <div className="min-h-screen bg-gray-50 overflow-x-hidden selection:bg-amber-200 selection:text-amber-900">
      ...
      <section className="relative overflow-hidden pt-24 lg:pt-36 pb-20">
        <div className="absolute inset-0 z-0 bg-[#0A1628]">
          {/* Abstract background mesh / particles (Subtle Framer Motion animation) */}
          <motion.div 
            ...
            style={{
              backgroundImage: 'radial-gradient(circle at 20% 30%, rgba(212, 175, 55, 0.15) 0%, transparent 40%), radial-gradient(circle at 80% 70%, rgba(30, 58, 95, 0.4) 0%, transparent 50%)',
        ...
        <div className="relative z-10 mx-auto max-w-6xl px-6">
          <div className="max-w-3xl">
            <motion.div 
              ...
              className="inline-flex items-center gap-2 rounded-full bg-white/5 border border-white/10 px-4 py-1.5 mb-8 backdrop-blur-sm"
            >
            ...
            >
              The Future of
              <span className="block mt-2" style={{ color: '#D4AF37', textShadow: '0 2px 20px rgba(212,175,55,0.2)' }}>Land Ownership</span>
              is Here
            </motion.h1>

            <motion.p 
              custom={3} initial="hidden" animate="visible" variants={heroVariants}
              className="text-lg lg:text-xl text-white/70 leading-relaxed mb-10 max-w-2xl font-light"
            >
            ...
            <motion.div custom={4} initial="hidden" animate="visible" variants={heroVariants} className="flex flex-wrap gap-5">
              <MagneticButton>
                <Link to="/register" className="flex items-center gap-2 rounded-lg px-8 py-4 text-base font-semibold text-[#0A1628] transition-all shadow-[0_4px_20px_rgba(212,175,55,0.3)] hover:shadow-[0_8px_30px_rgba(212,175,55,0.5)]"
                  style={{ background: 'linear-gradient(135deg, #D4AF37, #FDE047)' }}>
                  Get Started <ArrowRight className="h-4 w-4" />
                </Link>
              </MagneticButton>
              <MagneticButton>
                <Link to="/search" className="flex items-center gap-2 rounded-lg border border-white/20 px-8 py-4 text-base font-semibold text-white hover:bg-white/5 backdrop-blur-sm transition-all hover:border-white/40">
          ...
          {/* Hero stats (GSAP Animated) */}
          <div ref={statsRef} className="mt-20 grid grid-cols-2 gap-6 sm:grid-cols-4 relative z-10">
            {STATS.map(s => (
              <div key={s.label} className="stat-card rounded-xl bg-white/5 border border-white/10 p-5 backdrop-blur-md hover:bg-white/10 transition-colors duration-300">
      ...
      <section ref={trustRef} className="relative z-20 -mt-8 mx-auto max-w-6xl px-6">
        <div className="bg-white rounded-2xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] border border-gray-100 p-8 lg:p-12">
          <div className="grid grid-cols-2 gap-8 md:grid-cols-4">
            {TRUST_INDICATORS.map((item, i) => (
              <div key={i} className="trust-item flex flex-col items-center text-center gap-4">
                <div className="trust-icon flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-50/80 border border-blue-100 text-blue-800 shadow-sm">
      ...
      <section className="py-32 px-6 bg-gray-50 relative overflow-hidden" ref={timelineRef}>
        <div className="mx-auto max-w-4xl relative">
          <div className="text-center mb-20">
            <motion.div 
              ...
              className="inline-flex items-center gap-2 rounded-full bg-amber-50 border border-amber-200 px-4 py-1.5 mb-6"
            >
              <FileText className="h-4 w-4 text-amber-700" />
              <span className="text-sm font-semibold text-amber-800 uppercase tracking-wider">The Complete Process</span>
            </motion.div>
            <motion.h2 
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              ...
              className="font-serif text-4xl lg:text-5xl font-bold text-gray-900 mb-5"
            >
          ...
          <div className="relative pl-8 md:pl-0">
            {/* Scroll Progress Line using framer-motion useScroll on the container */}
          ...
          <div className="grid gap-8 md:grid-cols-3">
            {ROLE_CARDS.map((card, i) => (
              <motion.div
                ...
                className={`relative group bg-white rounded-2xl p-8 border border-gray-100 shadow-sm transition-all duration-300 ${card.shadow} ${card.borderHover}`}
              >
                <div className={`absolute top-0 left-0 w-full h-1 rounded-t-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-r from-transparent via-[var(--accent)] to-transparent`} style={{ '--accent': card.color.includes('green') ? '#15803d' : card.color.includes('blue') ? '#1e40af' : '#b45309' }} />
                
                <div className="text-5xl mb-6 bg-gray-50/50 w-20 h-20 flex items-center justify-center rounded-2xl border border-gray-100 group-hover:scale-110 transition-transform duration-300 ease-out">{card.emoji}</div>
                <h3 className="font-serif text-2xl font-bold text-gray-900 mb-3">{card.role}</h3>
      ...
      <section className="py-24 px-6 relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-3xl text-center z-10">
          ...
          <motion.p 
            ...
            viewport={{ once: true }}
            transition={{ delay: 0.2 }}
            className="text-white/70 mb-10 text-lg lg:text-xl font-light"
          >
          ...
          >
            <Link to="/register" className="flex items-center gap-2 rounded-lg px-8 py-4 text-base font-semibold text-[#0A1628] hover:shadow-[0_8px_30px_rgba(212,175,55,0.4)] transition-all"
              style={{ background: 'linear-gradient(135deg, #D4AF37, #FDE047)' }}>
              Start Registration <ArrowRight className="h-4 w-4" />
            </Link>
            <Link to="/login" className="flex items-center gap-2 rounded-lg border border-white/20 px-8 py-4 text-base font-semibold text-white hover:bg-white/10 transition-all">
              Sign In
            </Link>
          </motion.div>
        </div>
      ...
      <footer className="bg-white border-t border-gray-200 py-10 px-6">
        <div className="mx-auto max-w-6xl flex flex-col sm:flex-row items-center justify-between gap-6 text-sm text-gray-500 font-medium">
          ...
          <div className="flex gap-8">
            <a href="#" className="hover:text-[#0A1628] transition-colors">Privacy Policy</a>
            <a href="#" className="hover:text-[#0A1628] transition-colors">Terms of Service</a>
            <a href="#" className="hover:text-[#0A1628] transition-colors">Contact Support</a>
          </div>
        </div>
      </footer>
    </div>
      ...
      <div className={`w-full md:w-1/2 ${isEven ? 'md:pr-16' : 'md:pl-16'}`}>
        <div className="bg-white p-8 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-shadow duration-300 group">
          <div className="flex items-center justify-between mb-4">
            <span className="font-mono text-5xl font-black text-gray-100 group-hover:text-amber-50 transition-colors duration-300 absolute top-4 right-6 pointer-events-none">{step.step}</span>
            <div className="flex items-center gap-3 relative z-10">
              <div className="text-2xl bg-gray-50 w-12 h-12 flex items-center justify-center rounded-xl border border-gray-100">{step.icon}</div>
